...
export class AnalyticsService {  constructor() {
    ...
    if (!this.ordersCollection || !this.productsCollection || !this.usersCollection) {
      throw new Error('Failed to initialize Firestore collections');
    }
  }
  ...
  async getOverviewStats(): Promise<OverviewStats> {
    try {
      ...
      try {
        ...
        console.error('Error calculating some metrics:', error);
